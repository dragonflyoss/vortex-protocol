...
/// DownloadPersistentPiece represents a download persistent piece request.
/// Value Format:
...
/// ```
#[derive(Debug, Clone)]
pub struct DownloadPersistentPiece {
    task_id: String,
    piece_number: u32,
}
...
impl DownloadPersistentPiece {
    /// new creates a new DownloadPersistentPiece request.
    ...
    /// len returns the length of the download persistent piece request.
    pub fn len(&self) -> usize {
        TASK_ID_SIZE + PIECE_NUMBER_SIZE
    }

    /// is_empty returns whether the download persistent piece request is empty.
    pub fn is_empty(&self) -> bool {
        self.task_id.is_empty()
    }
}

/// Implement TryFrom<Bytes> for DownloadPersistentPiece for conversion from a byte slice.
impl TryFrom<Bytes> for DownloadPersistentPiece {
    type Error = Error;

    /// try_from decodes the download persistent piece request from the byte slice.
    fn try_from(bytes: Bytes) -> Result<Self> {
        if bytes.len() != TASK_ID_SIZE + PIECE_NUMBER_SIZE {
            return Err(Error::InvalidLength(format!(
                "expected {} bytes for DownloadPersistentPiece, got {}",
                TASK_ID_SIZE + PIECE_NUMBER_SIZE,
                bytes.len()
            )));
        }

        Ok(DownloadPersistentPiece {
...
/// Implement From<DownloadPersistentPiece> for Bytes for conversion to a byte slice.
...
mod tests {
    ...
    fn test_new() {
        ...
        let piece_number = 42;
        let download_persistent_piece = DownloadPersistentPiece::new(task_id.clone(), piece_number);

        assert_eq!(download_persistent_piece.task_id(), task_id);
        assert_eq!(download_persistent_piece.piece_number(), piece_number);
        assert_eq!(
            download_persistent_piece.len(),
            TASK_ID_SIZE + PIECE_NUMBER_SIZE
        );
    }
    ...
    #[test]
    fn test_is_empty() {
        let download_persistent_piece_empty = DownloadPersistentPiece::new("".to_string(), 0);
        let download_persistent_piece_non_empty = DownloadPersistentPiece::new("a".repeat(32), 1);

        assert!(download_persistent_piece_empty.is_empty());
        assert!(!download_persistent_piece_non_empty.is_empty());
    }
    ...
    fn test_valid_conversion() {
        ...
        let bytes: Bytes = download_persistent_piece.into();
        let download_persistent_piece = DownloadPersistentPiece::try_from(bytes).unwrap();
    ...
    fn test_invalid_conversion() {
        ...
        let result = DownloadPersistentPiece::try_from(invalid_bytes);
