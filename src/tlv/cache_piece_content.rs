...
impl CachePieceContent {
    ...
    /// is_empty returns whether the cache piece content request is empty.
    pub fn is_empty(&self) -> bool {
        self.metadata.length == 0
    }
}

/// Implement TryFrom<Bytes> for CachePieceContent for conversion from a byte slice.
impl TryFrom<Bytes> for CachePieceContent {
    type Error = Error;

    /// try_from decodes the cache piece content request from the byte slice.
...
/// Implement From<CachePieceContent> for Bytes for conversion to a byte slice.
...
/// CachePieceMetadata implements the CachePieceMetadata functions.
impl CachePieceMetadata {
    /// new creates a new CachePieceMetadata.
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        number: u32,
        offset: u64,
...
/// Implement TryFrom<Bytes> for CachePieceMetadata for conversion from a byte slice.
impl TryFrom<(Bytes, u32)> for CachePieceMetadata {
    type Error = Error;

    /// try_from decodes the cache piece metadata request from the byte slice.
...
/// Implement From<CachePieceMetadata> for Bytes for conversion to a byte slice.
...
mod tests {
    ...
    fn test_cache_piece_content_try_from_insufficient_bytes_for_metadata_length() {
        ...
        let result = CachePieceContent::try_from(short_bytes);
