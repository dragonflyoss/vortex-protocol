...
/// ```
#[derive(Debug, Clone)]
pub struct PersistentPieceContent {
    metadata_length: u32,
    metadata: PersistentPieceMetadata,
}
...
impl PersistentPieceContent {
    ...
    /// is_empty returns whether the persistent piece content request is empty.
    pub fn is_empty(&self) -> bool {
        self.metadata.length == 0
    }
}

/// Implement TryFrom<Bytes> for PersistentPieceContent for conversion from a byte slice.
impl TryFrom<Bytes> for PersistentPieceContent {
    type Error = Error;

    /// try_from decodes the persistent piece content request from the byte slice.
    fn try_from(bytes: Bytes) -> Result<Self> {
        ...
        if bytes.len() != METADATA_LENGTH_SIZE + metadata_length as usize {
            return Err(Error::InvalidPacket(format!(
                "expected {} bytes for PersistentPieceContent, got {}",
...
/// Implement From<PersistentPieceContent> for Bytes for conversion to a byte slice.
...
impl PersistentPieceMetadata {
    /// new creates a new PersistentPieceMetadata.
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        number: u32,
        offset: u64,
...
/// Implement TryFrom<Bytes> for PersistentPieceMetadata for conversion from a byte slice.
impl TryFrom<(Bytes, u32)> for PersistentPieceMetadata {
    type Error = Error;

    /// try_from decodes the persistent piece metadata request from the byte slice.
    fn try_from(input: (Bytes, u32)) -> Result<Self> {
        let (bytes, length) = input;
        if bytes.len() != length as usize {
            return Err(Error::InvalidLength(format!(
                "expected {} bytes for PersistentPieceMetadata, got {}",
                length,
                bytes.len()
            )));
        }
...
/// Implement From<PersistentPieceMetadata> for Bytes for conversion to a byte slice.
impl From<PersistentPieceMetadata> for (Bytes, u32) {
    /// from converts the persistent piece metadata request to a byte slice.
    fn from(metadata: PersistentPieceMetadata) -> (Bytes, u32) {
        let PersistentPieceMetadata {
            number,
            offset,
            length,
            digest,
...
mod tests {
    ...
    fn create_test_persistent_piece_content() -> PersistentPieceContent {
    ...
    fn create_test_metadata() -> PersistentPieceMetadata {
        PersistentPieceMetadata {
            number: 42,
            offset: 1024,
            length: 2048,
            digest: "a".repeat(32),
    ...
    #[test]
    fn test_persistent_piece_content_conversion_roundtrip() {
        let original = create_test_persistent_piece_content();
        let bytes = Bytes::from(original.clone());
        let result = PersistentPieceContent::try_from(bytes).unwrap();
    ...
    #[test]
    fn test_persistent_piece_content_try_from_insufficient_bytes_for_metadata_length() {
        let short_bytes = Bytes::from(vec![0u8; 4]); // Less than METADATA_LENGTH_SIZE (8)
        let result = PersistentPieceContent::try_from(short_bytes);
    ...
    #[test]
    fn test_persistent_piece_content_try_from_insufficient_metadata_bytes() {
        let mut bytes = BytesMut::new();
        bytes.put_u32(100); // metadata_length = 100
        bytes.put(&vec![0u8; 50][..]); // But only provide 50 bytes of metadata

        let result = PersistentPieceContent::try_from(bytes.freeze());
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), Error::InvalidPacket(_)));
    }

    #[test]
    fn test_persistent_piece_content_with_empty_parent_id() {
        ...
        let bytes = Bytes::from(persistent_piece_content.clone());
        let result = PersistentPieceContent::try_from(bytes).unwrap();
    ...
    #[test]
    fn test_persistent_piece_content_with_long_parent_id() {
    ...
    #[test]
    fn test_persistent_piece_content_with_zero_values() {
        let persistent_piece_content = PersistentPieceContent::new(
            0,
            0,
            0,
    ...
    #[test]
    fn test_persistent_piece_content_with_max_values() {
        let persistent_piece_content = PersistentPieceContent::new(
            u32::MAX,
            u64::MAX,
            u64::MAX,
    ...
    #[test]
    fn test_persistent_piece_content_metadata_length_calculation() {
        let persistent_piece_content = PersistentPieceContent::new(
            123,
            456,
            789,
        ...
        assert_eq!(persistent_piece_content.metadata_len(), expected_length);
    ...
    #[test]
    fn test_persistent_piece_content_with_short_digest() {
        let persistent_piece_content = PersistentPieceContent::new(
            1,
            0,
            100,
    ...
    #[test]
    fn test_persistent_piece_content_with_long_digest() {
    ...
    #[test]
    fn test_persistent_piece_content_bytes_structure() {
        let persistent_piece_content = create_test_persistent_piece_content();
        let bytes: Bytes = persistent_piece_content.clone().into();
        ...
        assert_eq!(metadata_length, persistent_piece_content.metadata_len());
        assert_eq!(bytes.len(), METADATA_LENGTH_SIZE + metadata_length as usize);
    }

    #[test]
    fn test_persistent_piece_content_new() {
        let persistent_piece_content = PersistentPieceContent::new(
            42,
            1024,
            2048,
        ...
        assert_eq!(persistent_piece_content.metadata().number, 42);
        assert_eq!(persistent_piece_content.metadata().offset, 1024);
        assert_eq!(persistent_piece_content.metadata().length, 2048);
        assert_eq!(persistent_piece_content.metadata().digest, "a".repeat(32));
        assert_eq!(
        ...
        assert_eq!(
            persistent_piece_content.metadata_len(),
            (NUMBER_SIZE
                + OFFSET_SIZE
                + LENGTH_SIZE
                + DIGEST_LENGTH_SIZE
                + persistent_piece_content.metadata().digest.len()
                + PARENT_ID_LENGTH_SIZE
                + persistent_piece_content.metadata().parent_id.len()
                + TRAFFIC_TYPE_SIZE
                + COST_SIZE
                + CREATED_AT_SIZE) as u32,
        );
    ...
    #[test]
    fn test_persistent_piece_content_is_empty() {
        let empty_persistent_piece = PersistentPieceContent::new(
            0,
            0,
            0,
            "a".repeat(32),
        ...
        let non_empty_persistent_piece = PersistentPieceContent::new(
            1,
            0,
            100,
            "a".repeat(32),
        ...
        assert!(empty_persistent_piece.is_empty());
        assert!(!non_empty_persistent_piece.is_empty());
    }

    #[test]
    fn test_persistent_piece_metadata_conversion_roundtrip() {
        let metadata = create_test_metadata();
        let (bytes, length) = <(Bytes, u32)>::from(metadata.clone());
        let result = PersistentPieceMetadata::try_from((bytes, length)).unwrap();
    ...
    #[test]
    fn test_persistent_piece_metadata_try_from_invalid_length() {
        let metadata = create_test_metadata();
        let (bytes, correct_length) = <(Bytes, u32)>::from(metadata);
        let wrong_length = correct_length + 10;
        let result = PersistentPieceMetadata::try_from((bytes, wrong_length));
    ...
    #[test]
    fn test_persistent_piece_metadata_try_from_too_short_bytes() {
        let short_bytes = Bytes::from(vec![0u8; 10]);
        let result = PersistentPieceMetadata::try_from((short_bytes, 10));
        assert!(result.is_err());
    }

    #[test]
    fn test_persistent_piece_metadata_with_empty_parent_id() {
        let metadata = PersistentPieceMetadata {
            number: 1,
            offset: 0,
            length: 100,
            digest: "b".repeat(32),
    ...
    #[test]
    fn test_persistent_piece_metadata_with_long_parent_id() {
    ...
    #[test]
    fn test_persistent_piece_metadata_with_zero_cost() {
        let metadata = PersistentPieceMetadata {
            number: 0,
            offset: 0,
            length: 0,
    ...
    #[test]
    fn test_persistent_piece_metadata_with_max_values() {
        let metadata = PersistentPieceMetadata {
            number: u32::MAX,
            offset: u64::MAX,
            length: u64::MAX,
    ...
    #[test]
    fn test_persistent_piece_metadata_invalid_utf8_in_digest() {
        let metadata_with_short_digest = PersistentPieceMetadata {
            number: 1,
            offset: 0,
            length: 100,
        ...
        let result = PersistentPieceMetadata::try_from((bytes, length));
        assert!(result.is_ok());
    }
}
